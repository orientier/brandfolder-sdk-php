...
class BrandfolderClient {
   ...
   *   $collections = $result->data;
   *   foreach ($collections as $collection) {
   *     echo "{$collection->attributes->name} ({$collection->id}): {$collection->attributes->asset_count} assets <br/>";
   *   }
   * }
   *
   * ```
   ...
   * $bf = new BrandfolderClient($api_key);
   *
   * $result = $bf->createCollectionInBrandfolder('My Collection', $brandfolder_id, 'The best collection ever!', 'my-coll');
   * if ($result) {
   ...
   *     echo "{$section->attributes->name} ({$section->id}). Default asset type: {$section->attributes->default_asset_type} <br/>";
   *   }
   * }
   *
   * ```
   ...
   */
  public function updateAsset(string $asset_id, ?string $name = NULL, ?string $description = NULL, ?array $attachments = NULL, string|int|\DateTime $availability_start = NULL, string|int|\DateTime $availability_end = NULL): object|false {
    $attributes = [];
    if (!is_null($name)) {
   ...
   *  accross an entire Org, you can: (1) list all Brandfolders
   ...
   *     echo "&nbsp; All values currently in use: " . !empty($values_list) ? implode(', ', $values_list) : 'None' . "<br/>";
   *   }
   * }
   *
   * // Example C.
  ...
  public function listInvitations(string $entity_type, string $entity_id, ?array $query_params = []): object|bool {
    ...
    switch ($entity_type) {
      ...
      default:
        $this->status = 0;
        $this->message = 'Could not determine endpoint for listing invitations. Please specify an Organization, Brandfolder, Collection, Portal, or Brandguide.';
   ...
   * $result = $bf->createInvitation('brandfolder', 'brandfolder-id-abc-123', $invitee_email, 'collaborator', 'Join us!');
   *
   * if ($result) {
   ...
   *    echo "Permission {$permission->id}: {$user->first_name} {$user->last_name} ({$user->email}) has permission level {$permission->attributes->permission_level} <br/>";
   *  }
   * }
   *
   * ```
   ...
   *   echo "Permission {$permission_result->data->id}: {$user->first_name} {$user->last_name} ({$user->email}) has permission level {$permission_result->data->attributes->permission_level} <br/>";
   * }
   *
   * ```
   *
